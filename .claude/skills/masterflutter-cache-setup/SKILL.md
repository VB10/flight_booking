---
name: masterflutter-cache-setup
description: >-
  ...
  example. Use when the user runs /masterflutter-cache-setup or asks to add a cache manager /
...
- User invokes **`/masterflutter-cache-setup`** or asks to add a cache/storage layer, Hive, or
...
### 6. Auth session as a DTO (uses `/masterflutter-cache-add-model`)
...
- Model codegen: **`/masterflutter-cache-add-model`**

## Invocation

- **`/masterflutter-cache-setup`** — or just describe the task; the
