...
description: >-
  ...
  Use when the user runs /masterflutter-cubit-add, asks for Cubit + mixin + ValueListenable pattern,
...
- User invokes **`/masterflutter-cubit-add`** or asks to add Cubit to a feature page.
...
- **`/masterflutter-cubit-add {feature_path}`** — or just describe the task; the
  description above triggers this skill automatically.

Copy this `.claude/skills/masterflutter-cubit-add/` folder into another project's `.claude/skills/` and keep or adapt `docs/prompt/flutter_cubit_feature_prompt.md` for portability.
