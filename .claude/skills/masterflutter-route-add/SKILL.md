...
description: >-
  ...
  auth_guard.dart if the route is public. Use when user runs /masterflutter-route-add
...
- User invokes **`/masterflutter-route-add`** or asks to add a new page/screen/route.
- The go_router navigation infrastructure already exists (`lib/product/navigation/`).

## Prerequisites

These must already exist (set up by `/masterflutter-route-setup`):
...
If they don't exist, run `/masterflutter-route-setup` first.
...
- **`/masterflutter-route-add {page_name} {path}`** — or just describe the task; the
